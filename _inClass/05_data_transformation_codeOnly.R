...
sqrt(2)^2 == 2     # => FALSE (due to approximate computation)
...
# it excludes _both_ FALSE and NA values. 
...
## [?]: From where to where did the 3 fastest flights go to?
...
x == y * (x %/% y) + (x %% y)  ## [?]: explain why OR mc == 0, 1, x, y.
...
## [?]: ggplot: plot curve of average delay per day/week/month
...
# [?]: Zoom in on 2 outliers with >50% of cancelled flights.
flights %>%
  mutate(cancelled = (is.na(dep_delay) & is.na(arr_delay))) %>%
  group_by(month, day) %>%
  summarise(n = n(),
...
# [?]: Dest. with fewer than .01% of flights?            
...
# [?]: Which destinations with at least 1000 arrivals has the worst arrival delays?
flights %>%
  group_by(dest) %>%
  summarise(n = n(),
            n_not_NA = sum(!is.na(arr_delay)),
