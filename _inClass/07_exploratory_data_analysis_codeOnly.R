...
# Solution: Compare density (count standardized to 1): 
...
#     number of points with varwidth = TRUE:
...
# Task: Find "outlier" temperatures (Definition: > 2 SD from group mean).

library(nycflights13)
weather

# 2 types of outliers:
# a) wrt. overall distribution
# b) wrt. monthly distribution

crit <- 2 # SDs from mean

# Temperature:
# degrees Celsius = (degrees Fahrenheidt - 32) x 5/9
...
gender <- sample(x = c(0, 1), size = 1000, replace = TRUE)
height <- rep(NA, 1000)
noise <- rnorm(mean = 0, sd = 10, n = 1000)
height[gender == 0] <- 165 + noise[gender == 0]
height[gender == 1] <- 180 + noise[gender == 1]
data <- as_tibble(cbind(gender, height))

data %>% 
  mutate(mn_all = mean(height)) %>%
  group_by(gender) %>%
  mutate(mn_gender = mean(height))
