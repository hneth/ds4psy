...
## Exercises using `p_info`

### Exercise 1: Explore age

Use `p_info` to explore distribution of `age` values by various other factors. 
...
Use `p_info` to explore distribution of `intervention` values by various other factors. 
...
ggplot(p_info) +
  ...
  labs(title = "Distribution of participants over intervention groups", 
...
Using `p_info`: Is the distribution of gender (variable `sex`) similar within the intervention groups?
...
## Exercises using `data_wide`

## Exercise 4: Scatterplots

Using `data_wide`: Initial scores of `ahiTotal.0` and `cesdTotal.0` and `age`?

Create two scatterplot to investigate if there is an systematic relationship between `age` and the initial `ahiTotal.0` scores/`cesdTotal.0` scores (i.e., before the intervention).

```{r exercise_4, echo = TRUE, eval = TRUE}
# Exercise 4: Create 2 scatterplots to investigate if there is a 
# systematic relationship between ahiTotal scores/cesdTotal scores and age.
...
ggplot(data_wide, mapping = aes(x = age, y = ahiTotal.0)) +
  geom_point(color = "steelblue", shape = 16, size = 1.5) +
  geom_smooth(color = "firebrick") +
  labs(title = "Age and ahiTotal scores before start of interventions", 
       x = "Age", y = "ahiTotal Score (pre-intervention)") + 
  theme_bw()

## (4b): cesdTotal-scores
ggplot(data_wide, mapping = aes(x = age, y = cesdTotal.0)) +
  geom_point(color = "forestgreen", shape = 16, size = 1.5) +
  geom_smooth(color = "firebrick") +
  labs(title = "Age and cesdTotal scores before start of interventions", 
       x = "Age", y = "cesdTotal Score (pre-intervention)") +
  theme_bw()
```

## Exercise 5: Violins

Using `data_wide`: Do the initial happiness scores (`ahiTotal.0`) or depression (`cesdTotal.0`) vary by the levels of `income`?
...
## Exercise 6: Scores at different occasions

Using `data_wide`: Explore `ahiTotal` and `cesdTotal` scores at different occasions. 
...
## (6a): Show the relationship of ahiTotal and cesdTotal at Occasion 0 using a scatterplot
...
## (6b): Add the trend-line to this plot
...
## (6c): Make this plot look nicer, for example by using color or changing the shape or size of points:
ggplot(data = data_wide) +
...
## (6d): Add information on intervention group using 2 different options
...
## (6e): Let's create 4 plots (hint: facet_grid) for occasion 5 and compare them to the one for occasion 0. 
## What has changed?
...
Trends of happiness and depression 

Let`s create plots of the improvement in `ahiTotal` scores and `cesdTotal` over the course of the study (occasion 0 - occasion 5) with information on the `intervention` groups.
...
Measuring improvements between occasions. 
...
data_wide %>% 
  ...
  summarise(mean_improve01_i = mean(improve01ahi, na.rm = TRUE),    
            mean_improve12_i = mean(improve12ahi, na.rm = TRUE),  # here is something interesting. 
...
ggplot(e) +
  ...
  labs(title = "Mean change in ahi scores for men and women", 
       x = "intervention",           y = "Change in ahi scores (mean)") +
  theme_light() +
...
ggplot(e) +
  ...
  labs(title = "Mean change in cesd scores for men and women", 
       x = "intervention", y = "Change in cesd scores (mean)") +
  theme_light() +
  geom_hline(yintercept = 0)
