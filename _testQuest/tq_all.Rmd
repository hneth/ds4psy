...
It also illustrates the procedure of our **mid-term exam** (on June 4, 2018). 
...
### Task `r nr<-nr+1; nr`: Using `PlantGrowth` data
...
**`r nr`d.** Reformat the `sleep` data in `sp` so that the 2 groups appear in 2 lines (rows) and 10 subject IDs as 10 columns (1). 
...
(or "partial election plots"): Own data on 2017 German election results
...
**`r nr`a.** Save the tibble `dplyr::starwars` as `sw` and report its dimensions (1). 
...
### Task `r nr<-nr+1; nr`: Weather flights 
...
Own data: Identifying outliers --- some outliers are more equal than others...

This task examines the statistical definition of outliers and uses a generated dataset (entitled `out.csv`) which you can obtain and load with the `read_csv()` command:

```{r, outlier_load_data, echo = TRUE, eval = TRUE}
## Load data: 
data <- read_csv("out.csv")  # read comma-separated-file
```
...
**`r nr`c.** What is the gender (or `sex`) distribution in this sample (1)? 

**`r nr`d.** Create a plot that shows the distribution of `height` values for each gender (1).
...
(i.e., individuals with `height` values deviating more than 2 SD from the overall mean of `height`) (1);
...
(i.e., individuals with `height` values deviating more than 2 SD from the mean `height` of their own gender) (1).
...
**`r nr`g.** Visualize the raw values and distributions of `height` for both types of outliers (`out_1` and `out_2`) in 2 separate plots and describe the `height` and `sex` combination of the individuals shown in each plot (2). 
...
## (g) Visualization of both types of outliers (out_1 and out_2): ----- 
...
pt <- pt + 10  # increment point total
```
...
**`r nr`b.** Transform `st` into a longer table `st_long` that contains 18 rows and only 1 numeric variable for all stock prices. Adjust this table so that the `day` and `time` appear as 2 separate columns (2). 
...
**`r nr`d.** Create a plot that shows the shape of the distribution of `Sepal.Width` values for each species (1). 

**`r nr`e.** Create a plot that shows `Petal.Width` as a function of `Sepal.Width` 
separately (i.e., in 3 facets) for each species (1).
...
## Note: Using raw values (ir_long) to visualize the distributions of val 
...
## (i) Turn ir_long back into original wide format using tidyr::spread -----
# ir_long
...
Create a (psychological) dataset `people` that allows illustrating contents from all chapters. 
