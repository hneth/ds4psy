...
data <- numeric()
# data <- readLines("http://www.apup.org/images/digits100000.txt")        
# orig. data source http://www.geom.uiuc.edu/~huberty/math5337/groupe/digits.html
data <- readLines("./data/_pi/pi_100k.txt")  # my version, based on http://www.geom.uiuc.edu/~huberty/math5337/groupe/digits.html
# head(data)  # is a string

N <- 10000  # set N

# subset of N digits: 
dataA <- numeric()
dataA <- paste0(substr(data, 1, 1), substr(data, 3, (N - 1))) # skip the "." at position 2!  
# head(dataA)
...
pal <- colorRampPalette(c(unikn.pal[[4]], grey(.25, 1)))  # on white
...
# par(bg = "black", mar = c(0, 0, 0, 0))
par(bg = "white", mar = rep(.25, 4))

plot(x = 0, y = 0, type = "n", 
     xlim = c(-50, 90), ylim = c(-100, 40),
     xlab = "", ylab = "", 
     xaxt = 'n', yaxt = 'n'
     )
...
- R graph gallery at <https://www.r-graph-gallery.com/portfolio/data-art/> 

- Inspiring creations by Antonio Sánchez Chinchón at <https://fronkonstin.com/>: 
