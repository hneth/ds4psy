...
<img src = "./pix/big/big_tile_rand_12_brd_lbl.png" alt = "ds4psy" style = "width: 400px; float: right; border:0;"/>
</a>
...
```{r logo_sketch, fig.width = 2, fig.height = 2}
dx <- 1
dy <- 1
dd <- 1
...
ggplot(ds4p) +
  geom_point(x = 0 , y = 0, pch = 4, size = 5, col = grey(.5, 1)) + 
  geom_text(aes(x = x, y = y, label = sym), size = 5, fontface = 2, adj = .5) + 
...
if (n_x == 1) {
  ...
  unikn_sort <- unikn.pal[c(4, 2, 10, 5)]   #  4 colors: seeblau, lightblau, white, black
} else if (n_x == 3) {
  unikn_sort <- unikn.pal[c(4:1, 10:8, 6:5)] #  9 colors: seeblau > white > black
} else {
  unikn_sort <- unikn.pal[c(4, 4:1, 10:5)]   # 11 colors: seeblau (2x) > white > black [default]
}
...
- Change order of sorted tiles (so that black is at bottom right corner). 
- Consider highlighting one row in a different color (e.g., gold to orange). 
...
```{r, echo = FALSE}
unikn_sort
unikn_pi <- unikn_sort[c(6, 11, 10, 1, 3:5, 9:7)]  # remap: 0 = white, 1 = black, 2 = seegrau4, 3 = seeblau, etc. 
unikn_pi  # 10 colors: black > white > seeblau
```
