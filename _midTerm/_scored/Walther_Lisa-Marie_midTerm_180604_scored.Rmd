...
# Correct, but why not insert values directly:
de <- tibble(Party = c("CDU/CSU", "SPD", "Others"),
             `Share 2013` = c(41.5, 25.7, 100 - (41.5 + 25.7)),
             `Share 2017`= c(33, 20.5, 100 - (33 + 20.5))
             )
de
...
# Good try, but start with de2 and use stat = "identity":
ggplot(de2, aes(x = share, y = result, fill = Party)) +
...
# Note: To see which individuals these are:
sw %>% 
...
# Consider using a different binwidth value.
...
# Good, but even simpler when using facet_wrap(~Species) with the above histogram:
ggplot(ir, aes(x = Sepal.Width, fill = Species)) +
  facet_wrap(~Species) +
  geom_histogram(binwidth = .1)+
  labs(title = "Distributions of Sepal.Width values by Species")
...
# Good, but try using multiple mutate steps in one pipe:
data_out <- data %>%      
...
# Total of 41 points (out of 50).
# Grade: 1.0.  Well done -- congratulations!  
