...
# Try using de_2 (and variable names not as character strings): 
de_2 %>%
  ggplot(mapping = aes(fill = party, y = value, x = key)) +
...
# [1]3 Personen sind weder M noch W
...
sw %>%
  filter(species == "Droid") %>%
  summarize(median = median(height, na.rm = TRUE),
...
# Good, but even clearer:
st_long %>%
  filter(time == "end") %>%
  ggplot(aes(x = day , y = stockprice, color = stock)  ) +
  geom_point(size = 4) +
...
# Note: Quotes are not needed.
# Good, but asked was: 
st_long %>%
  spread(key = time, value = stockprice)
...
# almost, missing group_by(Species)
# and change variable names (Petal parts?)
...
# One way to solve this is by combining multiple mutate steps in 1 long pipe:
