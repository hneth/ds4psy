...
# Minor adjustment (to get rid of NA values):
de <- tribble(
  ~Party, ~Share_2013, ~Share_2017,
  # -----|-------------|-----------
  "CDU/CSU", .415, .33,
  "SPD",     .257, .25,
  "Others", 1 - (.415 + .257), 1 - (.33 + .25)
)
...
# Use stat = "identity" to show numbers of the table: 
ggplot(data = de_2, aes(x = year, y = share, fill = Party)) +
...
# Almost: Count the number of column headers... 
...
# Note: Use the raw data (ir) to create these plots 
# (as the summary table im1 no longer shows distributions):
ir %>%
...
# Total of 26 points (out of 50).
# Grade: 1.7. Good, but keep getting better.   
