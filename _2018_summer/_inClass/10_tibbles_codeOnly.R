...
# Task: Mean Petal.Width by species:
it %>%
  group_by(Species) %>%
  summarise(n = n(),
...
# Checking the number of missing values (NA):
sum(is.na(it))  # => 0 (i.e., no missing values)
...
## [in.class]: Review from chapter 7: Exploratory Data Analysis (EDA) ------

## 6. What makes the distribution of diamonds$carat and flights$sched_dep_time similar?
##    (See Exercise 6 in Chapter 16.3.4: http://r4ds.had.co.nz/dates-and-times.html)

# Distribution of diamonds$carat:
ds <- filter(diamonds, carat < 3.1) # smaller subset
