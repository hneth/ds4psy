...
```{r plot_toc, fig.width = 8, fig.height = 8}
...
# Save plot:
cur_name <- paste0(pic_path, "toc_chronology", ".png")
...
# Save plot:
cur_name <- paste0(pic_path, "toc_relevance", ".png")
...
# Save plot:
cur_name <- paste0(pic_path, "centrality", ".png")
ggsave(cur_name, width = 15, height = 15, units = c("cm"), dpi = 300)


# (C) Highlight the current session (in gold):
nr_session <- 3  # current session nr (as in polar clock plot above)
...
cur_session_clock <- ggplot(toc, aes(x = nr)) +
  ...
  # scale_fill_gradientn(colors = unikn_toc) +
  scale_fill_gradient(low = "white", high = seeblau, limits = c(min_ctr, max_ctr)) +
  geom_text(aes(y = (max_ctr + .45 * tol), label = tp), adj = .5, size = txt_size) + 
  scale_x_continuous(breaks = 0:N, labels = 0:N) + 
  scale_y_continuous(limits = c(0, (max_ctr + tol)), breaks = 1:max_ctr, labels = 1:max_ctr) + 
  coord_polar() + 
  # theme_void() + 
...
# Save plot:
cur_name <- paste0(pic_path, "clock_", nr_session, ".png")
ggsave(cur_name, width = 15, height = 15, units = c("cm"), dpi = 300)
```
...
```{r harmonographs_adapted, eval = FALSE, fig.width = 5, fig.height = 5}
