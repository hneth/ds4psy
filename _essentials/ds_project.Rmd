...
What characterizes a good data science project? 
As the quality of any data exploration and analysis depends on (a) the quality of the data, (b) the questions asked, and (c) the analysis performed, it is difficult to establish a fixed set of rules for data science projects. Here are some general guidelines:

- Try asking an interesting question that can be answered by available data.

- Apply and show what you have learned (e.g., in various sessions of our course). 

- In short, find, use, transform, and visualize data to address an interesting question. 

## Components

### 1. Motivation

A motivated project should link questions with data. 
There are 2 principle ways to motivate a project:

1. Starting from questions: Ask some research question(s) that you hope to answer with data. 
Which data could address or answer the question? 

2. Starting from data: Why use this data? Which interesting questions can it inform or answer?

Note that the questions should be expressed without referring to the data (i.e., rather than 
"What is the correlation between variable `x` and variable `y`?" try a more general statement like  
"What is the relation between someone's mood and sports activities?"). 
...
- Amount: There are no limits or restrictions regarding data size, but larger datasets are better than smaller ones, data in several files better than 1, etc. 

- The data should include multiple and different types of variables (e.g., text, numeric, categorical, logical). 

- Include a codebook (overview over all variables, their types, and ranges).
...
- Results: Which answers does your analysis suggest?
...
Please submit a zipped-archive of an R-project, including all _scripts_ and _data_ (in `csv` format):[^2] 
...
An email with your attached project should be sent to <h.neth@uni.kn> [by **August 15**, 2019].
