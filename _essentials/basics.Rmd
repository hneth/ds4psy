...
This file contains a brief introduction into basic concepts and commands when learning R. Knowing some R is not a necessary precondition for reading and learning data science with our textbook ([r4ds](http://r4ds.had.co.nz)), 
...
```{r first_functions, echo = TRUE, eval = FALSE}
min(1, 2, -3, 4)
paste0("ab", "cd")
...
```{r objects, echo = TRUE, eval = FALSE}
...
```{r basic_objects, echo = TRUE, eval = FALSE}
...
```{r reassigning_objects, echo = TRUE, eval = FALSE}
...
```{r function_matching_object_types, echo = TRUE, eval = FALSE}
...
```{r arithmetic_functions, echo = TRUE, eval = FALSE}
...
The same arithmetic operators also work with numeric _vectors_ (see Exercise 2 below). (See `?Arithmetic` for help on arithmetic operators and `?Syntax` for a full list of precedence groups.) 
...
```{r logical_comparisons_operators, echo = TRUE, eval = FALSE}
## Logical comparisons:
2 > 1   # larger than
...
```{r creating_vectors, echo = TRUE, eval = FALSE}
# Creating vectors: 
c(1, 2, 3)
c(a, b)
...
```{r scalar_vectors, echo = TRUE, eval = FALSE}
a
is.vector(a)
length(a)
...
```{r vector_creation, echo = TRUE, eval = FALSE}
# Sequences (with sep):
...
```{r indexing_vectors, echo = TRUE, eval = FALSE}
...
```{r rectangular_data, echo = TRUE, eval = FALSE}
x <- 1:3
y <- 4:6
z <- 7:9
...
```{r data_frame, echo = TRUE, eval = FALSE}
...
```{r read_online_data, echo = TRUE, eval = FALSE}
library(readr)
...
**Exercise 1.** Evaluate and explain the following results (and correct any errors that may occur):
...
**Exercise 2.** With only a little knowledge of R you can perform quite fancy financial arithmetic. Assume that you have won an amount of EUR 1000 and are considering to deposit this amount into a new bank account that offers an annual interest rate of 0.1%. 
...
```{r ex2_financial, echo = FALSE, eval = FALSE}
# Definitions:
...
**Exercise 3.** When introducing arithmetic functions above, we showed that they can be used with numeric scalars (i.e., numeric objects with a length of 1). 
...
```{r ex3_arithmetic_vectors, echo = FALSE, eval = FALSE}
...
**Exercise 4.** Predit the result of the arithmetic expression `x %/% y * y + x %% y`. Then test your prediction by assigning some number to `x` and `y` and evaluating the expression. Finally, explain why the result occurs. 

```{r ex4_integer_arithmetic, echo = FALSE, eval = FALSE}
x <- 4711
y <- 1307
...
**Exercise 5.** Assume the following definitions for a survey: 
...
**Exercise 6.** Examine the participant information in `p_info` by describing each of its variables: 
...
```{r ex_p_info, echo = FALSE, eval = TRUE, results = "hide", fig.show = "hide"}
## Load data (again): 
p_info <- read_csv(file = "http://rpository.com/ds4psy/data/posPsy_participants.csv")
# p_info
...
- My favorite introduction to R is [YaRrr! The Pirate’s Guide to R](https://bookdown.org/ndphillips/YaRrr/), an ebook by Nathaniel Phillips that provides a more detailed and much more entertaining introduction to R.  (See [Rpository.com/learnR/](http://rpository.com/learnr/) for corresponding exercises with solutions.)
