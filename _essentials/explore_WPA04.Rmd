...
```{r ex_verify_participants, echo = TRUE, eval = TRUE}
...
3. Inspect `posPsy_wide` for missing values. Do you see some systematic pattern to them?

```{r ex_missing_values, echo = TRUE, eval = TRUE}
df <- posPsy_wide  # copy data
df
sum(is.na(df))  # 37440
...
2. Visualize the distributions of `income` levels overall and separately for each `intervention` 
(**Hints:** Use `dplyr` to compute a corresponding summary table before using calling `ggplot`. 
To create separate plots, use facet (e.g., `facet_wrap(~intervention)`) on the overall plot.) 
...
df %>% 
  ...
  ggplot(., aes(fill = factor(income))) + 
    geom_bar(aes(x = income, y = n), stat = "identity") +
...
3. Plot some initial variables (happiness or depression) by `income`.
