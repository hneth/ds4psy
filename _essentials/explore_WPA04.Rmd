...
Actually, our bar plots above (using either the raw data of `AHI_CESD` or our summary table `id_by_occ` to show the number of participants by occasion) expressed a trend: How many participants dropped out of the study from `occasion` 0 to 6. We can make the same point in a line plot: 
...
## Exercise 1: Explore `posPsy_wide` 
...
We checked the `age` and `sex` distributions of participants. But are the participants in the 4 `intervention` groups balanced in terms of their `income` levels? And could the initial scores of the participants at occasion 0 (i.e., before any intervention took place) vary as a function of their `income`? Let's find out... 
...
```{r ex2_income_counts, echo = TRUE, eval = FALSE}
## Data: 
df <- posPsy_wide
# df
...
```{r ex2_income_dist, echo = TRUE, eval = TRUE}
## Data: 
df <- posPsy_wide
# df
...
```{r ex2_income_scores, echo = TRUE, eval = TRUE}
## Data: 
df <- posPsy_wide
# df
...
**ToDo:** Re-sort dropout graphs above (from high to low number of measurements).

- Add question about selective dropouts above (point to Exercise). 

- Create a new variable: all vs. at least one missing occasion.
- Any group differences in age, sex, income, happiness or depression scores?



**Important Note:** 

Distinguish _exploration_ from _confirmation_: 
Beware of exploratory nature of EDA. Given the convenience of powerful software tools and the vast abundance of possible comparisons: There is a high chance of false positive results. 

So don't get too excited every time you spot an effect or pattern: Easy to kid others and yourself into thinking you found something. To _really_ find something, you need to predict and replicate it (ideally others too)!
...
```{r ex_changes_dist, echo = TRUE, eval = TRUE}
# Exercise 2:  
