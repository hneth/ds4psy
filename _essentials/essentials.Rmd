...
- Enter and run R commands in the Console window of RStudio. 
- Use R as a calculator for simple arithmetic.
- Assign values to named objects. 
- Call simple R functions. 
...
Data can be found in the form of individual data points (so-called _scalars_, which can be of different types) or longer sequences of values (_lists_ or _vectors_). However, most of the time we are dealing with datasets that contain multiple rows and columns (2-dimensional _matrices_ or _data frames_, or multi-dimensional _arrays_). 

Whenever working with rectangular data structures -- data consisting of multiple cases (rows) and multiple variables (columns) -- our first step in this course is to create or transform the data into a tibble. A _tibble_ is defined by the package `tibble` and implements a particular type of data table (or a simpler version of a _data frame_, which is the most common data structure in R). 
...
- See <https://www.r-graph-gallery.com/histogram/> and 
- <https://ggplot2.tidyverse.org/reference/> for more examples. 
...
- See <https://www.r-graph-gallery.com/scatterplot/> and 
- <https://ggplot2.tidyverse.org/reference/> for more examples. 
...
- See <https://www.r-graph-gallery.com/barplot/> and 
- <https://ggplot2.tidyverse.org/reference/> for more examples.
...
1. _Straight and curved lines_: 
When using lines to illustrate boundaries, limits, or trends in plots, we can add them by specifying their key parameters (e.g., their `intercept`, `slope`, etc.). 
...
2. _Drawing functions_: 
A more general approach to drawing lines is using functions that define the value of `y` as a computation on some value `x`:
...
3. _Line plots of data_: 
When we have grouped `data` (e.g., some values measured repeatedly over time) it often makes sense to show their development as a line plot. 
For instance, imagine having taken the following measurements of 3 people over the days of 1 week: 

```{r create_lineplot_data, echo = FALSE, eval = TRUE}
library(tidyverse)
library(knitr)
...
We can easily define this data as a tibble (e.g., row-by-row, using the `tribble` command), but then encounter a problem: 
To use `geom_line` we need to define a mapping from some variable `x` to some variable `y`. However, we do not have an individual variable `x` here, but rather 7 values of `x` for every person (for different days of the week). To obtain a single variable that contains all dependent values for `x`, we need to re-format the data from wide to long format (see [Chapter 12: Tidy data](http://r4ds.had.co.nz/tidy-data.html), which introduces the `tidyr` package). 

```{r lineplot_of_data_1, fig.show = "hold"}
...
Note that the labels on the x-axis are ordered alphabetically (i.e., from "Fri"" to "Wed"). 
The reason for this is that -- in `tb_long` -- the variable `day` is of type character.  
To fix this problem, we need to turn `day` into a _factor_ with levels that match its values: 

```{r lineplot_of_data_2}
...
- _colors_, _shapes_ and _sizes_ can be set withing geoms (and are variable when inside `aes(...)`, or fixed when set outside). Using color often involves setting specific color scales;  
- _labels_ are essential for understanding plots: `labs(...)` allows setting titles, captions, axis labels, etc.;  
- _legends_ can be crucial for understanding aesthetic mappings. They can be edited or (re-)moved;  
- _themes_ allow for a consistent look, can be selected and modified.  
...
## Advanced plots and related packages
...
- The [Masterlist](http://r-statistics.co/Top50-Ggplot2-Visualizations-MasterList-R-Code.html) (with code) at <http://r-statistics.co>

can inspire and provide many interesting pointers. The site 

- [ggplot2-exts.org](https://www.ggplot2-exts.org/) 

also provides valuable resources for `ggplot` users, as it shows packages specifically designed to work with `ggplot2`. In the following, we illustrate the packages `ggmosaic` and `ggalluvial` that allow showing the relations and transitions between categorical data (sets). 

### Example 1: Mosaic plots

An example showing the use of `ggmosaic`: 
...
# Good practice: 
# Use the dependent variable (or most important variable)
# as fill variable. 
...
# As usual, we can change where we define variables: 
...
### Example 2: Alluvial plot

Alluvial plots with data as a frequency table: 
...
Alluvial plots with data in long format:
...
#### Example 3: Parallel set diagram 
...
This section summarizes some essential parts of [Chapter 7: Exploratory data analysis (EDA)](http://r4ds.had.co.nz/exploratory-data-analysis.html). 


## Defining EDA 

In the introduction to data visualization, we mentioned that creating good graphs is both an art and a craft. 
This implies that a recipe for creating good graphs involves three ingredients: 
...
This recipe can be extended beyond graphs, as a mixture of the same ingredients is needed for all aspects of data analysis. 
For instance, when obtaining and exploring a new dataset, it is both an art and a craft to quickly obtain a good understanding of its contents. _Exploratory data analysis_ (EDA) is the process of getting a grasp of new data. Efficient and effective EDA requires combining commands on tibbles (`tibble`), data visualization (`ggplot2`), and data transformation (`dplyr`). 

## Basic questions

Getting a grasp of some data requires understanding two inter-related aspects: 

1. _Semantics_: What is the meaning and functional role of the observations? 
...
2. _Formats_: What data types are contained in the data and how are they arranged?  
    
    - How is the data formatted (in rows vs. columns)? 
    - What types of variables (columns) exist? 
    - Is the data tidy? (See the definition in [Chapter 12: Tidy data](http://r4ds.had.co.nz/tidy-data.html).)
...
Creating good graphs is both an art and a craft, but also allows a quick overview of an unknown set of data. 
...
[Chapter 12: Tidy data](http://r4ds.had.co.nz/tidy-data.html) teaches a consistent way to organise tabular data. It introduces commands of the `tidyr` package, which is a core member of the tidyverse. 
