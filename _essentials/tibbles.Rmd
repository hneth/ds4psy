...
Convert the data frames `datasets::attitude`, `datasets::mtcars`, and `datasets::Orange` into tibbles and inspect their dimensions and contents. 
What types of variables do they contain? What is the basic unit of an observation? 
...
Let's turn the `iris` data -- contained in R datasets -- into a tibble and conduct an EDA on it. 

Evaluate `?iris` to read a description of the dataset: `iris` provides the measurements (in cm) of plant parts (length and width of _sepal_ and _petal_ parts) for 50 flowers from each of 3 iris species (called _setosa_, _versicolor_, and _virginica_).
...
3. Create a histogram that shows the distribution of `Sepal.Width` values across the three species.
...
Ada, Brian, and Cecil are sharing a flat and keep a record of the items that each of them purchased for the household.  
...
Cecil | Fruit: \$6.30 | Batteries: \$6.10 | -- | Newspaper: \$2.90 | Honey: \$3.20  | Detergent: \$9.95   | --  |   

+++ here now +++

1. Which variables and which observations would you define here? 

2. Enter this data into a tibble `dec_1` and then answer the following questions (by using `dplyr` to create tables that contain the answer):
...
3. Interpret and re-create the following graphs (using `ggplot` and possibly `dplyr`):
  ...
  - Bar plots  
  - Cumulative amount paid per person over the week   
...
dec_1 <- tribble(
  ...
  "Ada", "Mon", "Butter", 2.00,
  "Ada", "Tue", "Pasta",  4.50,
  "Ada", "Wed", "Pencils",  3.25,
  "Ada", "Wed", "Cream",  3.90,
  "Ada", "Thu", "Milk",  3.80,
  "Ada", "Sat", "Cookies",  4.40,
  "Ada", "Sun", "Cake",  12.50,
  ...
  "Brian", "Sun", "Wine", 8.80,   
  "Cecil", "Mon", "Fruit", 6.30,
  "Cecil", "Tue", "Batteries", 6.10,
  "Cecil", "Thu", "Newspaper", 2.90,
  "Cecil", "Fri", "Honey", 3.20,
  "Cecil", "Sat", "Detergent", 9.95
)
dec_1
...
buy_by_day <- dec_1 %>%
  ...
  summarise(n = n(),
            sm_pay = sum(paid),
            mn_pay = sm_pay/n)
knitr::kable(buy_by_day)

# Arrange by overall expense: 
buy_by_day %>% 
  arrange(desc(sm_pay))
...
## Note: ------ 
# A more natural format would have been: 

buy  <- list("Ada", "Mon", "Bread", 2.50)  # by purchase

# but as vectors don't allow mixing data types (e.g., characters and numbers), 
# we entered the purchases as rows of a table/tibble.
