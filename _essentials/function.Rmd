...
Thus, are slightly more detailed template of a typical function is:

```{r function_basics_more, echo = TRUE, eval = FALSE}
name <- function(arg_1, arg_2 = FALSE) {
  
  <use arg_1 and arg_2 to solve a task>
...
In this template, `arg_1` is a mandatory argument (as no default is provided) and `arg_2` is an optional argument (with a default value of `TRUE`). The arguments are used to solve some task. Its result is then assigned to `result` and returned by `return(result)`. 
