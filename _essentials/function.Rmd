...
2. Which _inputs_ does the function take? Any input that the function needs to perform its task need to be supplied as a list of arguments `<args>`. Arguments typically include _data_ (e.g., scalars, vectors, or tables that the function uses) and additional _parameters_ (e.g., instructions on how something is to be done: Which type of calculation is desired? How should `NA` values be treated? Which color should be used? etc.). Any argument can be _optional_ (by including a default value that is used when no argument is provided) or _mandatory_ (by not including a default value).
