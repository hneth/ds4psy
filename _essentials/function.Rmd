...
If switch is used with a numeric expression `i`, it selects the `i`-th case. 
For example: 

```{r switch_numeric}
get_ith <- function(v, ith = 1) {
  
  switch(ith,
         v[1],
         v[2],
         v[3],
         stop("Unknown case of ith!")
  )
}
...
## But:
# get_ith(v, 4)  # would reach stop (and yield an Error)
```
...
- make sure that you choose good _names_ (for both functions and their arguments), 
