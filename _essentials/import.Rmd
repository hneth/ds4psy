...
Where does a _data frame_ or _tibble_ come from? If we don't enter it ourselves (e.g., with the `tibble` or `tribble` commands from our session on [Tibbles](http://rpository.com/ds4psy/essentials/tibbles.html)) we need to import it from an external source. 
This session discusses some essential options and pitfalls when using the tidyverse package `readr` for data import. 
`readr` provides a fast and friendly way to read rectangular data files (like `csv`, `tsv`, and `fwf`). 
