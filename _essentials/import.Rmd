...
```{r ex1_getwd, echo = FALSE, eval = FALSE}
my_wd <- getwd()
my_wd
...
```{r ex1_setwd, echo = FALSE, eval = FALSE}
setwd("./../data")
getwd()
list.files()
```
...
```{r ex1_setwd_back, echo = FALSE, eval = FALSE}
setwd(my_wd)
getwd()
```
...
```{r birthday, echo = FALSE, eval = FALSE}
...
The following data files are all variants of our data at <http://rpository.com/ds4psy/data/falsePosPsy_all.csv>:

a. <http://rpository.com/ds4psy/data/ex1.csv> (uses `,` as variable separator and `.` as decimal mark);  
b. <http://rpository.com/ds4psy/data/ex2.csv> (uses `;` as variable separator and `,` as decimal mark); 
c. <http://rpository.com/ds4psy/data/ex3.csv>  
d. <http://rpository.com/ds4psy/data/ex4.csv>  

1. Inspect each file and then read it in 2 ways (by using either `read.csv` or the appropriate variant of `read_csv`) and check whether both files are equal in each case.

```{r}
# Paths to all 4 files:
ex1 <- "http://rpository.com/ds4psy/data/ex1.csv"  
ex2 <- "http://rpository.com/ds4psy/data/ex2.csv"  
ex3 <- "http://rpository.com/ds4psy/data/ex3.csv"  
ex4 <- "http://rpository.com/ds4psy/data/ex4.csv" 
...
all.equal(as_tibble(a_1), a_2)  # `cond` is a factor in x and a character in y

# b: ex2 is separated by semi-colons (csv2):
b_1 <- read.csv(ex2, sep = ";", dec = ",")
b_2 <- read_csv2(ex2)

all.equal(as_tibble(b_1), b_2)  # `cond` is a factor in x and a character in y
