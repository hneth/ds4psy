...
```{r ex5_create_data, echo = FALSE, eval = TRUE}
...
all.equal(summary, summary_2a)  # TRUE
...
- Using a `csv` file is still the best choice. However, since our friend is supposed to be in Germany, we use a `csv2` file that adheres to the European convention that variables are separated by semi-colons (`;`) and the decimal mark is a comma (`,`) -- which is the default setting of `readr::write_csv2`.
...
# (a) write_csv:
readr::write_csv2(x = summary, path = "./../data/summary_a.csv2")
...
- As they are simple and functional, the `readr` functions `write_csv` and `write_csv2` provide good and safe options for exchanging files. 
