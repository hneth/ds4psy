...
This file contains **essential commands** from [Chapter 11: Data import](https://r4ds.had.co.nz/data-import.html) of the textbook [r4ds](http://r4ds.had.co.nz) and corresponding examples and exercises. 
...
Where does data (e.g., a _data frame_ or _tibble_) come from? If we don't enter it ourselves (e.g., with the `tibble` or `tribble` commands (see our session on [Tibbles](http://rpository.com/ds4psy/essentials/tibbles.html)) we typically import it from an external source. The scope of such sources is vast and we only cover the most common candidates: Data that is already stored in text form or other file formats that can easily be coerced into linear or rectangular data structures. 

This session discusses some essential options and pitfalls when using the tidyverse package `readr` for data import. `readr` provides a fast and friendly way to read vectors and rectangular data files (like `csv`, `tsv`, and `fwf`). 
...
v3 <- random_symbols(n = n, set = as.character(0:9), len = 8)
v4 <- random_symbols(n = n, set = letters, len = 6)  
...
read_delim(my_file, delim = ".", col_names = FALSE)

read_fwf(my_file, fwf_cols(ini = c(1, 2), 
                           age = c(4, 5), 
                           nr = c(7, 14), 
                           pwd = c(16, 21) ))
```
