...
# Using stats::reshape
data_long <- reshape(data_wide, varying = list(c = c("c1", "c2"),
                                               t = paste0("t", 1:2),
                                               p = paste0("p", 1:2)),
                     direction = "long",
                     v.names = c("c", "t", "p"), # wenn man in dem Schritt auch gleich umbennen will
                     timevar = "p",              # wenn position den Zeitpunkt angibt (bei mir nicht, da eine Zufallszahl
                     idvar = "subj")             # wird ansonsten neu eingefügt.

data_long %>% arrange(subj)
...
- Plotting one DV against many others: 

```{r explain_gather_plot}
