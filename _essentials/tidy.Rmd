...
```{r ex_create_exp_data, echo = FALSE, eval = TRUE}
...
t_2 <- t_1 %>%
  unite("sex:age", gender, age, sep = ":") %>%
  mutate(first_task = task_1) %>%
  unite("ct+st", color_time, shape_time, sep = "+") %>%
  select(name, `sex:age`, first_task, color, shape, `ct+st`)
t_2

# t_3: ---- 
# An even longer (more distributed) version of t_1 (Unipark style): 
