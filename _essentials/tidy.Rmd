...
Recreate the above bar plot using `ggplot2` with `data = table1`. 
...
2. Alternatively, split `table8` into 2 tidy sub-tables (one for `cases` and one for `population` counts), and then combine both sub-tables: 
...
# Either by using join commands of dplyr (see Chapter on "Relational data"): 
...
3. Using the base R command `reshape` also solves our problem (and even allows combining multiple blocks of DVs at once), but requires some adjustments to recover the `year` variable 
...
As always, there are many alternative ways to achieve a result -- which makes it absolutely crucial that we know which particular result we want to achieve.  
