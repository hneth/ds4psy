...
The answers are that `r target` occurs `r count` times in `pi_1000`. Its 1st occurence is at position `r target_positions[1]` and its `r count`-th occurrence at position `r target_positions[count]`.
...
- Solution: `while` loop, with a `condition` to stop the loop. 
