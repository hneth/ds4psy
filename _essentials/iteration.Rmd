...
1. Implement a function `dice` that uses the **base** R function `sample()` to simulate a throw of a dice (i.e., yielding each integer from 1 and 6 with equal probability). 
...
ggplot(tb) + 
  ...
  geom_text(label = paste0("mean = ", round(mean(out), 2)), x = mean(out) + 20, y = 1000, color = Bordeaux) +    # mean label
  geom_vline(xintercept = median(out), linetype = 2, color = Petrol) +  # median line 
  geom_text(label = paste0("median = ", round(median(out), 2)), x = median(out) + 20, y = 1100, color = Petrol) +  # mean label
  # Text and formatting: 
