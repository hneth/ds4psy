...
### Using a `for` loop {#iter:for-loop}
...
### For loop variations {#iter:for-loop-variations} 
...
## Functional programming {#iter:functional-programming}

### For loops vs. functionals

In R, `for` loops are not as important as in most other programming languages, because R is a _functional_ programming language. 
This means that it is possible to replace many `for` loops by wrapping up the body of a `for` loop in a function, and then call that function. 

#### Example {-}

Consider a simple data table `df`: 

```{r iter-functional-data}
# Data:
set.seed(1)  # for reproducible results
...
The standard solution for this task is using a `for` loop:

```{r iter-standard-loop}
...
But now we are no longer satisfied with the _mean_, but also want the _median_ and _standard deviation_ of every column. 
Of course we could write analog functions `col_median()` and `col_sd()`, but they would only differ in 1&nbsp;line from `col_mean` above. 
Hence, what we really would want is that we could _pass a function_ (like `mean` or `median`) to another function --- which is exactly what we will learn to do next. 
...
The 3 functions only vary in their last digit. Given this degree of duplicated code, we would make the last element an _additional argument_ to a more general function:
...
We can achieve exactly the same thing for 3 similar functions --- like `col_mean()`, `col_median()`, and `col_sd()` --- by adding an argument to a function that supplies the _function_ to apply to each column:
...
The idea of passing a _function_ (like `mean` or `median`) as an _argument_ (here: `fun`) to another _function_ (here: `col_summary`) is an extremely powerful idea, 
and it one reason for calling R a _functional_ programming language.


## Replacing loops by `apply` and **purrr** {#iter:apply-purrr}
...
Note that using `apply` or `map` is not necessarily faster than using `for` loops. 
The chief benefit of using these functions is not speed, but _clarity_: They make code easier to write and read.

### Using `apply` {#iter:apply}

The `base::apply` family of functions in (i.e., `apply`, `lapply`, `tapply`, etc.) replaces `for` loops by applying a function to designated parts of a (rectangular) data structure. To accomplish this, `apply` takes the following arguments: 
...
In this call, `MARGIN = 2` instructed `apply` to apply some function `FUN` to each _column_ of `X`. 
Changing the `MARGIN` argument `MARGIN = 1` will apply `FUN` to each _row_ of `X`: 
...
### Using **purrr** {#iter:purrr}

The **purrr** package provides modern and more consistent versions of `apply`. 

The main goal of using **purrr** functions (instead of `for` loops) is to break common list manipulation challenges into smaller and independent pieces. 
...
Each function takes a vector `.x` as input, applies a function `.f` to each element, and then returns a new vector that’s the same length (and has the same names) as the input vector. The type of the output vector is determined by the suffix of the `map` function (e.g., `map_chr` would return the output as a character vector).
...
- Predict the output of the following variants, then check your predictions:

```{r iter-map-variants, eval = FALSE}
map(df, mean)
map_int(df, mean)
map_lgl(df, mean)
map_chr(df, mean)
...
- Without the suffix `_dbl`,  `map(df, sd)` returns a _list_ (with an element for every column of `df`). 
...
- `map` also uses the generic `...` argument to allow passing additional arguments to the function indicated by `.f`: 

```{r iter-map-dotdotdot}
map_dbl(df, mean, trim = 0.5)
map_dbl(df, sd, na.rm = FALSE)
```

- `map` preserves the names of `.x`:
...
We could do this using the _shorthand for anonymous functions_:

```{r iter-map_split_map_2}
models %>% 
  map(summary) %>% 
  map_dbl(~.$r.squared)
```

But extracting named components from a list of elements is a common operation, so **purrr** provides an even shorter shortcut: We can use a _string_:

```{r iter-map_split_map_named}
models %>% 
  map(summary) %>% 
  map_dbl("r.squared")
```

Alternatively, we can also use an _integer_ to select elements by their position:

```{r iter-map_split_map_number}
x <- list(list(1, 2, 3), 
          list(4, 5, 6), 
          list(7, 8, 9))
...
### Comparing `map` vs. `apply` {#iter:compare-map-vs-apply}
...
### Advanced aspects of **purrr**

See the following sections of [r4ds]() for more advanced issues in combination of the `map` functions of **purrr**: 

- [21.6: Dealing with failure](https://r4ds.had.co.nz/iteration.html#dealing-with-failure) for using the adverbs `safely`, `possibly`, and `quietly` in combination with `map` functions.

- [21.7: Mapping over multiple arguments](https://r4ds.had.co.nz/iteration.html#mapping-over-multiple-arguments) for the `map2` variants, as well as `invoke_map`. 

- [21.8: Walk](https://r4ds.had.co.nz/iteration.html#walk) to use `walk`, `walk2` and `pwalk` for calling functions for their side effects. 

Rather than digging deeper into the functional programming paradigm of **purrr**, the following exercises focus on good-old fashioned `for` loops and the more essential aspects of `apply` or `map`. 

<!-- +++ here now +++ --> 


# Exercises (WPA10) {#iter:ex}

Here are the exercises of WPA10 and possible solutions for them.

## Exercise 1 {#iter:ex01}

#### Fibonacci loop and functions {-}

1. Look up the term _Fibonacci numbers_ and use a `for` loop to create a numeric vector of the first 25 Fibonacci numbers (for a series of numbers starting with `0, 1`).
...
1. According to [Wikipedia](https://en.wikipedia.org/wiki/Fibonacci_number), a Fibonacci sequence is the integer sequence of `0, 1, 1, 2, 3, 5, 8, ...`. 

Thus, the first 2 elements $e_{1}$ and $e_{2}$ of the series need to be provided. 
For $i > 2$, each element $e_{i}$ is the sum of the 2&nbsp;preceding elements: 

$e_{i} = e_{i-2} + e_{i-1}$.   

We turn this into a `for` loop as follows: 

```{r iter-ex1-fibonacci-1}
...
2. Incorporating the `for` loop into a function `fibonacci(n)`: 

```{r iter-ex1-fibonacci-2a}
...
3. Generalizing `fibonacci(n)` to a function `fibonacci(e1, e2, n)` is simple: 
The following version makes the arguments `e1` and `e2` optional to return the standard sequence by default. 

```{r iter-ex1-fibonacci-3}
...
```{r iter-ex1-fibonacci-3-check1}
fibonacci(n = 25)
```

but now also allows specifying different initial elements:

```{r iter-ex1-fibonacci-3-check2}
fibonacci(e1 = 1, e2 = 3, n = 25)
```


## Exercise 2 {#iter:ex02}

#### Looping for divisors

1. Write a `for` loop that prints out all positive divisors of the numbers 1000.  
...
Our first `for` loop required `N = 1000` iterations. 
However, realizing that the largest divisor cannot exceed `N/2` (for even numbers), and `N\3` (for odd numbers), we could achieve the same results with far fewer iterations: 
...
4. Use your `divisors` function to answer the question: 
Does the number&nbsp;1001 have fewer or more divisors than the number&nbsp;1000?
...
5. Use your `divisors` function and another `for` loop to answer the question: 
Which prime numbers exist between the number&nbsp;111 and the number&nbsp;1111?
...
- The loop above uses the `divisors` function within a `for` loop (in the range `range_min:range_max`). 
As the `divisors` function also uses a `for` loop to find all divisors of `N`, we are using a loop inside a loop. 
As such structures can quickly become very inefficient, it is a good idea to try reducing the number of iterations when possible. 

- The condition `length(divisors(i)) == 2` would _fail_ to detect a prime number `i=1`. 
A more general solution would first define an `is_prime` function and then use `is_prime(i)` in the `if` statement of the `for` loop: 
...
The `is_prime` function can be written in many different ways, of course. 
Check out this [stackoverflow thread](https://stackoverflow.com/questions/19767408/prime-number-function-in-r) for solutions.

## Exercise 3 {#iter:ex03}
...
**Note:** This exercise is based on **Exercise 1** of [Chapter 21.5.3](https://r4ds.had.co.nz/iteration.html#exercises-58) in [r4ds](https://r4ds.had.co.nz/). 
...
## Exercise 5 {#iter:ex05}

#### Z-transforming tables {-}

In this exercise, we will standardize an entire table of data (using a `for` loop, an `apply`, and a `map` function). 
...
In case you are not familiar with the notion of a _z score_ or _standard score_, look up these terms (e.g., [on Wikipedia](https://en.wikipedia.org/wiki/Standard_score)). 
...
z_trans <- function(v) {
    ...
    if (!is.numeric(v)) {
      message("v is not numeric: Leaving as is...")
      return(v)
    }
...
z_trans(v = c(-10:10))
z_trans(v = c(-1, NA, 1))
z_trans(v = letters[1:5])
z_trans(v = (1:4 > 2))
```

2. Load the _false positive psychology_ data (at <http://rpository.com/ds4psy/data/falsePosPsy_all.csv>) into `falsePosPsy` and remove any non-numeric variables from it. 

```{r iter-ex05-2a}
falsePosPsy <- read_csv(file = "http://rpository.com/ds4psy/data/falsePosPsy_all.csv")
falsePosPsy
```
...
```{r iter-ex05-2b}
# (a) Solve for individual variables: 
is.numeric(falsePosPsy$kalimba)
is.numeric(falsePosPsy$cond)

# (b) Generalize to entire table:
numeric_cols <- falsePosPsy %>% map_lgl(is.numeric)
numeric_cols
```
...
```{r iter-ex05-2c}
# Indexing columns of falsePosPsy by numeric_cols:
fpp_numeric <- falsePosPsy[ , numeric_cols]
# fpp_numeric

# Alternative solution:
fpp_numeric_1 <- falsePosPsy[, map_lgl(falsePosPsy, is.numeric)]
all.equal(fpp_numeric, fpp_numeric_1)

# Using dplyr::select:
fpp_numeric_2 <- falsePosPsy %>% dplyr::select(-cond)  # notice that `cond` is non-numeric
all.equal(fpp_numeric, fpp_numeric_2)
```
...
# for loop: 
for (i in seq_along(fpp_numeric)){
  
  out <- cbind(out, z_trans(fpp_numeric[[i]]))
...
Note that we use `cbind` rather than `c` within the `for` loop to add the results of `z_trans` to `out`. 
This is because `z_trans` returns a _vector_ for every column of `only_numeric`. 
Alternatively, we also could have constructed a very long vector (with a length of `nrow(fpp_numeric) x ncol(fpp_numeric)` = `r nrow(fpp_numeric)` x `r ncol(fpp_numeric)` = `r nrow(fpp_numeric) * ncol(fpp_numeric)`) and turned it into a rectangular table later.  

3. Repeat the task of 2. (i.e., applying `z_trans` to all numeric columns of `falsePosPsy`) by using the **base** R `apply` function, rather than a `for` loop. 
Save and print your resulting data structure as a tibble `out_2`.  

**Hint:** Remember to set the `MARGIN` argument to apply `z_trans` over all _columns_, rather than _rows_. 

```{r iter-ex05-3}
# Data:
# fpp_numeric

out_2 <- apply(X = fpp_numeric, MARGIN = 2, FUN = z_trans)

# Print result (as tibble):
out_2 <- as_tibble(out_2)
out_2
```

4. Repeat the task of 2. and 3. (i.e., applying `z_trans` to all numeric columns of `falsePosPsy`) by using an appropriate version of a `map` function from the **purrr** package. 
Save and print your resulting data structure as a tibble `out_3`. 

**Hint:** Note that the desired output structure is a rectangular data table, which is also a _list_.

```{r iter-ex05-4}
# Data:
# fpp_numeric

# Using map to return a list: 
out_3 <- purrr::map(.x = fpp_numeric, .f = z_trans)

# Print result (as tibble):
out_3 <- as_tibble(out_3)
out_3
```

5. Use `all.equal` to verify that your results of 2., 3. and 4. (i.e., `out_1`, `out_2`, and  `out_3`) are all equal.

**Hint:** If a tibble `t1` lacks variable names, you can add those of another tibble `t2` by assigning `names(t1) <- names(t2)`. 

```{r iter-ex05-5}
# Note that out_1 did not retain the variable names of `fpp_numeric`:
names(out_1) <- names(fpp_numeric)  # re-adding names to out_1
...
- The book [Advanced R](http://adv-r.had.co.nz/) contains several chapters on functional programming. 
