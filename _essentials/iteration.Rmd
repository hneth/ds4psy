...
Write code that uses one of the `apply` or `map` functions to:
...
apply(X = ggplot2::diamonds, MARGIN = 2, FUN = typeof) # Note: all viewed as characters!
...
How can you create a single vector that --- for each column in a data frame --- indicates whether or not it is a variable of some type (e.g., a factor, numeric variable, etc.)?
...
2. Load the _false positive psychology_ data (at <http://rpository.com/ds4psy/data/falsePosPsy_all.csv>) into `falsePosPsy` and remove any non-numeric variables from it. Then use `z_trans` in combination with a `for` loop to z-transform all of its (numeric) columns. 
Turn your resulting data structure into a tibble and print it. 
...
Selecting only the numeric columns into a new tibble `fpp_numeric`: 

```{r iter-ex05-2b}
fpp_numeric <- falsePosPsy %>% dplyr::select(-cond)  # notice that `cond` is non-numeric
fpp_numeric

# An automatic (but more complicated way) to select all numeric columns:
fpp_numeric_2 <- falsePosPsy[, map_lgl(falsePosPsy, is.numeric)]
all.equal(fpp_numeric, fpp_numeric_2)
```

Using a `for` loop to apply `z_trans` on any column of `fpp_numeric`:
...
Showing the equality of all 3 results:

```{r iter-ex05-5}
# Note that out_1 did not retain variable names:
names(out_1) <- names(out_2)
