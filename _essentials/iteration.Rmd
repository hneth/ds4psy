...
The syntax for creating an _anonymous function_ in R is quite verbose, so `purrr` provides a convenient shortcut: a _one-sided formula_.
...
Here, `.` is used as a pronoun that refers to the current list element. 

When we’re looking at many models, we might want to extract a summary statistic like the $R^{2}$. 
