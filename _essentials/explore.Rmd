...
This principle is short version of the following: 

- Know (the dimensions of) your data, (the types of) your variables, and (the meaning of) your observations. 
...
- **Principle 5:** Know and deal with missing values. 

In R, missing values are identified by `NA`. Note that `NA` is different from `NULL`: `NULL` represents the null object (i.e., something is undefined). By contrast, `NA` means that some value is absent:
...
the validity of cases and values (adding filter variables), and the distributions of variables. 
