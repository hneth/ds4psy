...
# Table 2: 
# n, ahiTotal_mn, and cesdTotal_mn by occasion x intervention: 
summary <- df %>%
...
Note the fancy color gradient of the last tile plot: The lowest average scores of happiness (or `ahiTotal_mn`) are shown in `"black"`, the highest ones in `"gold"`. 

A similar way of expressing the value of a continuous variable -- like a frequency `n` or an average score `ahiTotal_mn` -- is by mapping it to the size dimension of a point. In the following, we create the same plot twice: first from raw data (`df` and `geom_count`) and then from our summary table (`summary` and `geom_point` with `size` mapped to the frequency `n`):
...
However, this probably gets too confusing --- so we should better use 2 separate plots when expressing multiple relationships. This brings us to:

- **Principle 9:** Any plot should be as clear as possible, so don't try doing too much in one plot! 
