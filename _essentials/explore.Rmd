...
While adhering to this principle may seem unnecessary or trivial at first, it gets more important as analyses get longer and more complicated (e.g., distributed over multiple datasets and scripts) and when sharing your data or scripts with others (or with your future self). 

- **Principle 2:** Start with a clean slate and explicitly load all data and all packages required in your analysis.

Although RStudio IDE provides options for saving your workspace and for loading data or packages by clicking buttons or checking boxes, doing so renders the process of your analysis intransparent for anyone not observing all your actions. To be sure that others and you is able to repeat the same sequence of steps tomorrow or next year, it is a good idea to always start with a clean slate and make it clear which data and packages are required for the current analysis (unless there are good reasons to deviate from them):
