...
We have been using functions in all sessions of this course. Many of them -- like `c()`, `max()`, and `mean()` -- are included in R (so-called _base R_ functions), and the others -- like `mutate()`, `gather()`, and `ggplot()` -- were defined in additional packages (e.g., the tidyverse packages `dplyr`, `tidyr`, and `ggplot2`). Actually, the fact that most packages mostly provide additional functions shows that functions are a pretty big deal in R. Like tools, functions extend the scope of tasks that can be solved. To use a tool, we typically do not need to understand how it was designed and built. However, to create new tools in R, we need to know how to write functions. 
...
2. Which _inputs_ does the function take? Any input that is necessary for the function to perform its task must to be supplied as a list of arguments `<args>`. 
    ...
    - Any argument can be _optional_ (by including a default value that is used when no argument is provided) or _mandatory_ (by not including a default value). It is common to make data arguments mandatory, and use optional arguments for details. 
...
3. The `<body>` of a function uses the _inputs_ provided by `<args>` to perform the task for which the function is created. 
    ...
    - Although some functions are only used for their side-effects (e.g., load or save an object, create a graph, etc.), most functions are created to (also) return some sort of _output_ (e.g., some computed `result`). This can be done by calling the special function `return` in the function `<body>` (e.g., `return(result)`), typically as its last statement.[^2] Even when the function does not contain an explicit `return` statement, it returns the result of the last expression evaluated in the function. 
    
    - The _output_ of a function can assume many different data types (scalar, vector, list, table, etc.). 
    For instance, the result of `1:3 + 3:1` is a vector ``r 1:3 + 3:1``.
...
When doing these checks, examples that use unusual inputs (like `NA`, `NULL`, or arguments of different types) are particularly informative. 
...
# Note that the function also works for vectors: 
power(x = 1:5, exp = 2) 
power(x = 2,   exp = 1:4)
...
power(1:5, 2)
power(2, 1:4)
# However, when arguments are named: 
power(exp = 3, x = 2)  # order is irrelevant.
# => Safest way: Always provide argument names!
...
## Errors:
# power()       # 1st argument is mandatory
# power("A")    # setting x to non-numeric argument
# power(3, "B") # setting exp to non-numeric argument
```

#### Explicit `return`

A more explicit version of the same function would be:
...
Test this last statement by evaluating the `power_joke` function and trying to obtain the `"Just kidding"` line as an output. 
Then comment out the line `return(result)` and re-run the same checks. 
...
# => "Just kidding" is always reached 
#    (unless an earlier error occurs). 
```
...
- The function first checks if `v` is `NA`: If so, it returns `v` (i.e., `NA`).

- The function then checks if `v` is non-numeric: If so, it prints a message and returns `v`. 

- The function then computes 7 different statistical measures. This illustrates that functions can use other functions. For some of these functions (e.g., `mean`), the `describe` function passes the value of its optional argument `rm_na` to another function's `na.rm` argument. However, for another function (`quantile`), the `describe` function does not use its `rm_na` argument, but always provides `na.rm = TRUE`. 

- The function then creates a vector `out` that includes the 7 computed measures in a specific order and names the vector. 

- The function then returns the vector `out`. 

- Overall, the task addressed by `describe` is to provide a range of descriptive statistics of a vector `v`. 

#### Checking the function

Here are some possible ways of checking how `describe` works: 
...
Actually, `describe` is -- apart from subtle differences -- similar to the base R function `summary`: 
...
Writing good functions is a life-long aspiration. For starters, 

- make sure that you choose good _names_ for both your functions and their arguments; 

- _format_ your functions so that their structure is clear and they are easy to read (i.e., use blank lines between different parts and automatic indentation);  

- provide _comments_ to explain the function's purpose and anything else that may be difficult to understand.  


## Flow control 

Whereas most of our scripts so far relied on being executed linearly (in top-down fashion, line-by-line fashion),
using functions implies jumping around in large amounts of code.[^3] 
In addition, writing new functions often requires controlling the flow of information _within_ the body of a function. 
We can distinguish between several ways of doing this:

[^3]: Strictly speaking, we have also been using statements that were processed from left-to-right (e.g., assignments like `x <- 1`) or bottom-to-top (e.g., when assigning a multi-line pipe of `dplyr` statements to an object). Also, given that we have been using functions all along, we have been jumping around in base R code ever since the 1st session.


- Special functions (e.g., like `return`, `print`, or `stop`) cause side-effects or skip code by exiting functions.

- Many functions rely on _iteration_ and _loops_, which are covered in [Chapter 21: Iteration](https://r4ds.had.co.nz/iteration.html).

- Testing input arguments or distinguishing between several cases requires the _conditional_ execution of code, which is discussed next.


# Conditionals

As we have just seen in `describe`, functions frequently require checking some properties of its inputs, 
distinguishing between cases, and controlling the flow of data processing based on test results. 

## If-then

A test can be conducted by a _conditional_ statement, like the following if-then or `if (test) {...}` structure: 

```{r if_then, eval = FALSE}
...
If a test fails, we often want something _else_ to happen. 
To accommodate this desire, a slightly more complicated form of `if` statement includes an additional `{...}` after an `else` statement:  

```{r if_then_else, eval = FALSE}
if (test) { 
...
Here's a way to fix our problem from above (by providing a more comprehensive test): 
...
Here, `op` was specified as a character variable. 
If switch is used with a numeric expression `i`, it selects the `i`-th case (with `i` being coerced into an integer). 
For example: 
...
```{r nursery_rhyme_ugly, echo = TRUE}
...
The function appears to implement some nursery rhyme, but is really messy, unfortunately. 
Hence, we need to clean it up before trying to understand it. 
...
```{r nursery_rhyme_pretty, echo = TRUE}
check_flow <- function(n) {
...
2. Describe and try to understand this function. What does it do and how does it do it?
...
4. Test your predictions by evaluating the following calls:

```{r nursery_rhyme_calls, eval = FALSE}
# Check: 
check_flow(2 < 1)
check_flow(2)
check_flow(4)
...
Imagine someone proudly presents you the following 3 functions. Each of them takes a vector `v` as an input and tries to perform a simple task. For each function:
...
Test your function by running `feed_me("apples")`, etc. Here's a template with some blanks, to get you started: 
...
3. Extend your function to a `feed_vegan` function that uses 2 additional arguments: 
...
1. Explain what the following function `describe` (not to be confused with `describe` above) _intends_ to do and why it fails in doing it. 
...
This exercise writes a function for creating a specific type of plot. 
...
- See the [contributed cheatsheets](https://www.rstudio.com/resources/cheatsheets/) on _Base R_ and _Advanced R_. 

- [Chapter 19: Functions](https://r4ds.had.co.nz/functions.html) of [r4ds](https://r4ds.had.co.nz/) provides some additional remarks on the special dot-dot-dot argument (`...`), return values, pipeable functions, and the notion of a function's _environment_ or the _scope_ of variables. 

- To really understand how functions are evaluated and used in R, [Advanced R](http://adv-r.had.co.nz/) devotes chapters on [functions](http://adv-r.had.co.nz/Functions.html), on [environments](http://adv-r.had.co.nz/Environments.html), and 
on [functional programming](http://adv-r.had.co.nz/Functional-programming.html). 

- The `dplyr` [vignette on programming](https://dplyr.tidyverse.org/articles/programming.html) (see `vignette("programming")`) explains some specifics that matter for writing tidyverse functions.
