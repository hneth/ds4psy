---
title: "Tidying data, with WPA07 (ds4psy)"
...
```{r stock_solution, echo = FALSE, eval = FALSE, fig.show = "hold"}
# library(tidyverse)
...
```{r ex_solution_wide_to_long, echo = FALSE, eval = FALSE}
my_tbl_3 <- my_tbl_2 %>% 
...
```{r ex_solution_long_to_wide, echo = FALSE, eval = FALSE}
my_tbl_4 <- my_tbl_3 %>%
...
```{r ex4_long2wide, echo = FALSE, eval = FALSE}
## Load data: ---- 
...
```{r ex4_wide2long, echo = FALSE, eval = FALSE}
## Load data: ---- 
...
This exercise relies on the datasets for the [false positive psychology](http://rpository.com/ds4psy/essentials/datasets.html#false-positive-psychology) project: 

- `falsePosPsy_all` (78 x 19 variables): <http://rpository.com/ds4psy/data/falsePosPsy_all.csv>

```{r ex5_import_data, echo = FALSE, eval = TRUE, cache = FALSE, message = FALSE, warning = FALSE}
# Import the Dataset
...
(**Hint**: As both ages are in 2 separate variables, you need to use `gather` to collect ages in 1 variable.)

```{r ex5_2, echo = FALSE, eval = FALSE}
df %>%
  gather(key = "person", value = "age", dad, mom) %>%
  select(study:aged365, person, age) %>%
  ggplot(aes(x = age, fill = person)) +
...
(**Hint**: Again, you need to use `gather` to collect ages in 1 variable.)  

```{r ex5_3, echo = FALSE, eval = FALSE}
df %>%
  gather(key = "person", value = "age", dad, mom) %>%
  select(study:aged365, person, age) %>%
...
<!-- ADDENDUM: additional material (NOT USED HERE): 
