---
title: "Transforming data, with WPA03 (ds4psy)"
...
```{r group_by_ex, echo = TRUE, eval = TRUE}
sws %>%
...
```{r, starwars_transformations_2, echo = FALSE, eval = FALSE}
## Missing data: ----- 
...
```{r, starwars_transformations_3, echo = FALSE, eval = FALSE}
## (c) Gender issues: ----- 
...
```{r, starwars_transformations_4, echo = FALSE, eval = FALSE}
## (d) Homeworld issues: ----- 
...
```{r, starwars_transformations_5, echo = FALSE, eval = FALSE}
...
```{r, starwars_transformations_6, echo = FALSE, eval = FALSE}
## Bonus questions: ----- 
...
```{r ex_msleep, echo = TRUE, eval = FALSE}
## Data: 
# ?msleep  # check variables     
sp <- ggplot2::msleep
```

```{r, check_msleep, echo = FALSE, eval = FALSE}
# Check: 
...
```{r ex_msleep_subsets, echo = FALSE, eval = FALSE}
# Arranging rows:
sp1 <- sp %>%
...
```{r ex_msleep_mutate, echo = FALSE, eval = FALSE}
...
```{r outlier_solution_1, include = FALSE, fig.show = "asis"}
...
```{r outlier_solution_2, include = FALSE, fig.show = "asis"}
## Defining different outliers: ----- 
...
```{r outlier_solution_3, include = FALSE, fig.show = "asis"}
...
In [Exercise 6](http://rpository.com/ds4psy/essentials/basics.html#exercise-6) of [WPA01](http://rpository.com/ds4psy/essentials/basics.html#exercises-wpa01) and 
[Exercise 5](http://rpository.com/ds4psy/essentials/visualize.html#exercise-5) of [WPA02](http://rpository.com/ds4psy/essentials/visualize.html#exercises-wpa02) you used the `p_info` data (available at  <http://rpository.com/ds4psy/data/posPsy_participants.csv>) from 
...
```{r ex4_read_posPsy_data, echo = TRUE, eval = FALSE}
library(readr)
...
For instance, the following questions from [Exercise 6 of WPA01](http://rpository.com/ds4psy/essentials/basics.html#exercise-6):  

1. How many individuals are contained in the dataset? 
2. What percentage of them is female (i.e., has a `sex` value of 1)?

could be solved by using `dplyr` as follows:

```{r ex4_read_posPsy_demo, echo = TRUE, eval = FALSE}
# 1. How many individuals are contained in the dataset? 
N <- nrow(p_info) 
N
