...
See the textbook [Chapter 3: Data visualization](http://r4ds.had.co.nz/data-visualisation.html)) and [Chapter 7: Exploratory data analysis (EDA)](http://r4ds.had.co.nz/exploratory-data-analysis.html) and the links provided below for more detailed information. 
...
- `ggplot` requires data and maps independent variables to dimensions (e.g., the x- and y-axis) and dependent variables to geometric objects (called "geoms"). It typically assumes that the to-be-plotted data is in long format and contains independent variables as factors. 

- Different geoms can be combined, but their order matters (as later layers are printed on top of earlier ones).

- In `ggplot`, multiple commands are combined by `+`, rather than `%>%`. 
