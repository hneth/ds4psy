...
One way of viewing this table is as a series of columns. Each column consists of a variable name and (here: 5) values (which are of different types: numbers, characters, or Boolean truth values, and may or may not contain missing values).
...
fm <- tibble(
  ...
  name     = c("Adam", "Eva", "Xihi", "Yota", "Zack"), 
...
**Check**: 
If `tibble` and `tribble` were alternative commands, then the contents of our objects `fm` and `fm2` should be identical: 
...
Once we have an object as a tibble, we often want to access individual parts of it. 
Let's distinguish between 3 simple cases: 
...
More advanced operations on tibbles are covered in Data transformation (e.g., by using the `dplyr` commands `arrange`, `filter`, and `select`).


## Additional information

- Study `vignette("tibble")` and `?tibble`
- [Chapter 10: Tibbles](http://r4ds.had.co.nz/tibbles.html) 
