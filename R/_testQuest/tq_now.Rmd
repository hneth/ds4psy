...
An _outlier_ can be defined as someone whose value in some metric deviates by more than 2 SD from the mean of a reference group. Depending on the reference group, outliers can be computed in defined in (at least) 2 ways: 

1. relative to the _overall sample_ mean and SD;  

2. relative to _some subgroup's_ mean and SD.  
...
noise_1 <- round(rnorm(n, mean = 0, sd = 10), 0)
...
# data
mean(data$height) # => 174.782 (with seed 123)
```
...
B. Identify people who are outliers relative to _both_ the entire sample _and_ the sample of their own sex.
...
# (A) Compute means, SDs, and outliers of overall and by gender: -----  
data_out <- data %>%
...
# (B) Show outliers relative to entire population AND to their own group (sex):
out_1 <- data_out %>%
  filter(out_height & out_sex_height)
# out_1

# (C) Show outliers relative to own sex, but NOT relative to entire population:
out_2 <- data_out %>%
  filter(!out_height & out_sex_height)
# out_2
