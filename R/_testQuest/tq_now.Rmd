...
- **Submit** your script as an attachment to an _email_ (with the subject header "ds4psy: final exam") to <h.neth@uni.kn> by Wednesday (July 18th, 2018). 
...
**`r nr`a.** Report the data dimensions and number and percentage of missing values. (1&nbsp;point)
...
**`r nr`c.** Create a new variable `summerborn` that is `TRUE` when a participant was born in summer (defined as April to September) and `FALSE` when a person was born in winter (October to March).  (1&nbsp;point)


(B) Assessing IVs: 

**`r nr`d.** Compute the current `age` of each participant (in years, taking into account today's date). (2&nbsp;points) 

**`r nr`e.** List the frequency of each blood type (`blood_type`) (a) _overall_ and (b) by _gender_. (2&nbsp;points) 
...
**`r nr`g.** Plot the distributions of `height` (a) by _gender_ and (b) by _cohort_. (2&nbsp;points)


(C) Assessing DVs: 

**`r nr`h.** Compute the aggregate BNT score (the sum of all four `bnt_i` values, varying from 0 to 4 points) for each participant and check whether it varies systematically as a function of _gender_. (2&nbsp;points)
...
**`r nr`j.** Plot the relationship between `g_iq` and `s_iq`. (1&nbsp;point)

**`r nr`k.** Assess possible effects of _numeracy_ (as measured by `BNT`) on the 2 measures of intelligence (`g_iq` and `s_iq`). (2&nbsp;points)

**`r nr`l.** Assess possible effects of the variables 
...
b. graphically (by plotting means and/or distributions).  (10&nbsp;points)

```{r numeracy_solution}
# (a) inspect data: 
dim(data) # => 1000 participants x 12 variables
sum(is.na(data))   # 130 missing values
mean(is.na(data))  # 1.083 percent of missing data
