...
```{r graphics}
# Defining colors: 
seeblau <- rgb(0, 169, 224, maxColorValue = 255) # seeblau.4
...
```{r, plantGrowth, echo = TRUE}
# ?PlantGrowth

df <- as_tibble(PlantGrowth)
df

# Compute number of observations by group, their mean, median and standard deviation: 
df %>%
  group_by(group) %>%
  summarise(count = n(),
...
# Plot median and raw values of weight by group: 
...
# Compute average extra time by group:
df %>% 
  group_by(group) %>%
  summarize(n = n(),
            md = median(extra),
...
# Visualize raw values and group averages:
ggplot(df, aes(x = group, y = extra, fill = group)) +
  geom_boxplot() +
  # geom_violin() +
  geom_point(aes(shape = group), size = 3)

# Reformat data so that the 2 groups appear in 2 lines, and IDs as 10 columns:
df %>%
...
Plot a bar chart (using the data from tidyr::table1) that shows the number of cases per country (on the y-axis) as a function of the year (on the x-axis).
...
- compute new group-level variables with grouped mutates
...
# (c) Use `h_m` to list the 3 species with the smallest individuals (in terms of mean height)?
h_m %>% arrange(mn_height) %>% slice(1:3)

# (d) Use `h_m` to list the 3 species with the heaviest individuals (in terms of median mass)?
...
- view relationships between (categorical/continuous) variables
...
- create tibbles from tabular data
...
- gather wide datasets into longer ones
- spread long datasets into wider ones
...
## However, spreading from long to wider format would yield an 
